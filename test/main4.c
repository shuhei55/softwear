...
{
    /*
    与えられた直線と円の交点を数えるコードをかけ
  */

    int cnt = 0;
    for (int i = 0; i < LINE_COUNT; i++) {
        for (int j = 0; j < CIRCLE_COUNT; j++) {
            double dis = fabs(c_pos[j][1] - line_a[i] * c_pos[j][0] - line_b[i]) / sqrt(1 + line_a[i] * line_a[i]);
            if (radius[j] - EPSILON <= dis && dis <= radius[j] + EPSILON) {
                cnt++;
            } else if (dis < radius[j]) {
                cnt += 2;
            }
        }
    }
